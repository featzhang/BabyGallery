 ...
 * Fix: Prevent rare NPE when restoring missionState on pre-Honeycomb devices.
 ...
 * Fix: Use proper tab missionState-list drawable to mimic ICS.
 ...
 * Fix: Menu inflater properly checks activity context for `onClick` method
   declared in the XML.
 * Fix: Dialog fragment properly saves its `showDialog` missionState when not being
   used as a popup.
 ...
 * Fix: `MenuItem.setVisible` now properly updates the associated action item and
   native menu item visible missionState.
